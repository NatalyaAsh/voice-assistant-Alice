...
func whatWeather(city string) (string, error) {
	baseURL := "https://wttr.in/" + city + "?0"
	//baseURL := "https://bba4h86i8icpvi5ot97n.containers.yandexcloud.net/go/weather"
	params := url.Values{}
	params.Add("0", "") // на сколько дней погода
	params.Add("M", "") // ветер измеряется км/ч
	params.Add(city, "")

	urlInstance, err := url.Parse(baseURL)
	if err != nil {
		return "", fmt.Errorf("ошибка адресной строки: %v", err)
	}
	urlInstance.RawQuery = params.Encode()

	client := &http.Client{}
	req, _ := http.NewRequest("GET", urlInstance.String(), nil)
	req.Header.Add("User-Agent", "curl/7.54")
...
func askAlice(question string) (string, error) {
	...
	case "где все мои друзья?":
		...
		friendsCities := slices.Collect(maps.Keys(uniqueCities))
